...
def create_account(username: str, password: str, confirm: str, db_path: str) -> str:
    ...
    Raises AccountValidationError on an empty username, an empty password, a
    password shorter than auth.users._MIN_PASSWORD_LENGTH, or a
    password/confirm mismatch (no DB access happens on these paths), and lets
...
def main() -> int:
    ...
    def on_submit() -> None:
        try:
            ...
            return
        messagebox.showinfo("Account created", f"{username_entry.get()}: created")
        root.destroy()
